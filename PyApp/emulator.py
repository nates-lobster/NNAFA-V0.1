...
async def start_ws():
    ...
    async with websockets.serve(control_handler, "127.0.0.1", 8766):
        ...
        while True:
            # Ghost Process Watchdog [EMU]-[004]
            # Shutdown if no client connected for > 60s (after initial startup)
            if connected_clients == 0 and (time.time() - start_time) > 60.0:
                log("WATCHDOG: No active connections for 60s. Shutting down.")
                sys.exit(0)
            if connected_clients > 0:
                start_time = time.time() # Reset timer if client is connected
